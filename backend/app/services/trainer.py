...
class Trainer:
        ...
        try:
            ...
            if task in ["IMAGE_CLASSIFICATION", "OBJECT_DETECTION", "VISUAL_WAKE_WORDS"]:
                ...
                train_ds = tf.keras.utils.image_dataset_from_directory(
                    dataset_dir, validation_split=session["validation_split"],
                    subset="training", seed=123, color_mode=color_mode,
                    image_size=(img_height, img_width), batch_size=session["batch_size"],
                )
                val_ds = tf.keras.utils.image_dataset_from_directory(
                    dataset_dir, validation_split=session["validation_split"],
                    subset="validation", seed=123, color_mode=color_mode,
                    image_size=(img_height, img_width), batch_size=session["batch_size"],
                )
            ...
            loss_fn = (
                "binary_crossentropy" if task == "VISUAL_WAKE_WORDS"
            ...
            model.fit(
                ...
                epochs=session["total_epochs"], callbacks=callbacks,
            )
    ...
    def _prepare_audio_dataset(self, dataset_dir: str, batch_size: int,
        ...
        train_ds = tf.data.Dataset.from_tensor_slices(
            (X[:split_idx], y[:split_idx])).batch(batch_size)
